...
def admin_user(update: Update, context: CallbackContext):
    ...
    try:
        ...
        auth.authorize_admin(target_user_id)
        update.message.reply_text(f"User {target_user_id} has been authorized as admin.")
    except (IndexError, ValueError):
...
def unadmin_user(update: Update, context: CallbackContext):
    user_id = update.message.from_user.id
    ...
    try:
        target_user_id = int(context.args[0])
        auth.revoke_admin(target_user_id)
        update.message.reply_text(f"Admin access for user {target_user_id} has been revoked.")
    except (IndexError, ValueError):
        update.message.reply_text("Usage: /unauth <user_id>")
        return
...
def authorized_admins(update: Update, context: CallbackContext):
    user_id = update.message.from_user.id
    ...
    table = auth.get_authorized_admins_table()
    update.message.reply_text(f"Authorized admins:\n<pre>{table}</pre>", parse_mode=ParseMode.HTML)
...
dispatcher.add_handler(CommandHandler("admin", admin_user))
dispatcher.add_handler(CommandHandler("unadmin", unadmin_user))
dispatcher.add_handler(CommandHandler("authorized_admins", authorized_admins))
